...
import { aoDisplayName } from "@/games/animeorigin/aoUnitMeta";
import { AOUnitThumb } from "./AOUnitThumb";

interface AOAccountTableProps {
  players: PlayerData[];
  serverTime: number;
  skew?: number;
...
export function AOAccountTable({
  players,
  serverTime,
  skew = 0,
  sortField,
      ...
      <div className="overflow-x-auto pb-1">
        <div style={{ minWidth: innerMinWidth }} className="space-y-2">
          ...
          >
            ...
            <button onClick={() => handleSort("aoExp")} className="flex min-w-0 items-center justify-center gap-1 whitespace-nowrap hover:text-zinc-300">
              <i className="fa-solid fa-star text-[10px]"></i> Exp <SortCaret active={sortField === "aoExp"} dir={sortDir} />
            </button>
            ...
            <button onClick={() => handleSort("unitsTotal")} className="flex min-w-0 items-center justify-center gap-1 whitespace-nowrap hover:text-zinc-300">
              <i className="fa-solid fa-users text-[10px]"></i> Towers <SortCaret active={sortField === "unitsTotal"} dir={sortDir} />
            </button>
          ...
          {paged.map((p) => {
            ...
            // ช่องที่ถืออยู่จริง (EquippedTowers resolve ด้วย uuid) — payload เก่าไม่มี equippedUnits ตกไป top ตาม stars
            ...
            return (
              ...
              >
                ...
                <span title={`${formatAbbreviated(p.aoExp ?? 0)} EXP`} className="flex w-full min-w-0 items-center justify-center gap-1.5 overflow-hidden whitespace-nowrap rounded-md border border-emerald-500/20 bg-emerald-500/[0.07] px-2 py-2 text-[13px] font-bold tabular-nums text-emerald-300">
                  <i className="fa-solid fa-star shrink-0 text-[10px]"></i>
                  <span className="min-w-0 truncate">{formatAbbreviated(p.aoExp ?? 0)}</span>
                </span>
                <span title={formatAbbreviated(p.gems ?? 0)} className="flex w-full min-w-0 items-center justify-center gap-1 overflow-hidden whitespace-nowrap rounded-md border border-violet-500/20 bg-violet-500/[0.07] px-2 py-2 text-[13px] font-bold tabular-nums text-violet-200">
                  <img src="/images/anime-origin/icons/gems.png" alt="Gem" loading="lazy" className="h-4 w-4 shrink-0 object-contain" />
                ...
                <span title={formatAbbreviated(p.money ?? 0)} className="flex w-full min-w-0 items-center justify-center gap-1 overflow-hidden whitespace-nowrap rounded-md border border-amber-500/20 bg-amber-500/[0.07] px-2 py-2 text-[13px] font-bold tabular-nums text-amber-300">
                  <img src="/images/anime-origin/icons/gold.png" alt="Gold" loading="lazy" className="h-4 w-4 shrink-0 object-contain" />
                ...
                <span title={formatCount(p.currencies?.TraitReroll ?? 0)} className="flex w-full min-w-0 items-center justify-center gap-1 overflow-hidden whitespace-nowrap rounded-md border border-orange-500/20 bg-orange-500/[0.07] px-2 py-2 text-[13px] font-bold tabular-nums text-orange-200">
                  <img src="/images/anime-origin/icons/traitreroll.png" alt="Trait Reroll" loading="lazy" className="h-4 w-4 shrink-0 object-contain" />
                    ...
                    <span className="flex min-w-0 items-center justify-center rounded-md border border-line bg-white/[0.02] px-2 py-1">
                      <span className="flex shrink-0 items-center">
                        {shown.map((u, i) => (
                          <span
                            key={u.uuid}
                            title={`${aoDisplayName(u.asset, u.displayName)} ★${u.ascension ?? u.level} Lv${u.level}${u.trait ? ` · ${u.trait}` : ""}`}
                      ...
                      <span title={`${unitsCount} towers`} className="ml-1.5 shrink-0 rounded-md border border-amber-400/50 bg-amber-400/20 px-1.5 py-0.5 text-[11px] font-black tabular-nums text-amber-200">
                        {unitsCount}
                      </span>
                    </span>
                  )}
