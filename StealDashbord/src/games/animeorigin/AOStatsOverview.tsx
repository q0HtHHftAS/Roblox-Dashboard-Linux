...
import { AOCurrencyPicker } from "./AOCurrencyPicker";
import {
  AO_DEFAULT_CARDS,
  AO_OVERVIEW_CARDS_KEY,
  collectAOCurrencies,
  defaultCardOrder,
  type AOCurrencyEntry,
} from "./aoCurrencies";

function loadStoredOrder(): string[] | null {
  try {
    const raw = window.localStorage.getItem(AO_OVERVIEW_CARDS_KEY);
    if (!raw) return null;
...
function AOOverviewCardBody({ entry }: { entry: AOCurrencyEntry }) {
  return (
    <>
...
export function AOStatsOverview({ players }: { players: PlayerData[] }) {
  const entries = useMemo(() => collectAOCurrencies(players), [players]);
  ...
  const [order, setOrder] = useState<string[]>(() => [...AO_DEFAULT_CARDS]);
  useEffect(() => {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(AO_OVERVIEW_CARDS_KEY, JSON.stringify(order));
    } catch {}
  }, [order]);
  ...
  return (
    <section>
      ...
      {ghost && ghostEntry && (
        ...
        >
          <AOOverviewCardBody entry={ghostEntry} />
        </div>
      )}
