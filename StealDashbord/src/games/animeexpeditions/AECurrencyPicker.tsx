...
import type { AECurrencyEntry } from "./aeCurrencies";

interface AECurrencyPickerProps {
  open: boolean;
  onClose: () => void;
  entries: AECurrencyEntry[];
  trackedIds: string[];
  onToggle: (id: string) => void;
}

// Modal เลือก currency เข้า overview: แถวที่ track อยู่โชว์ × แดง (กดเอาออก) ที่เหลือโชว์ + เทา (กดเพิ่ม)
export function AECurrencyPicker({ open, onClose, entries, trackedIds, onToggle }: AECurrencyPickerProps) {
