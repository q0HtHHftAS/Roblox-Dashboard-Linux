...
export default function AnimeExpeditionsPage() {
  ...
  const [selectedGroup, setSelectedGroup] = useState<AEUnitGroup | null>(null);
  const [selectedSkin, setSelectedSkin] = useState<AESkinGroup | null>(null);
  ...
  // Faz2: กรองเฉพาะ payload ของเกมนี้ (legacy ไม่มี gameId นับเป็น stealanegg ไม่ปนมา)
  const aePlayers = useMemo(() => players.filter((p) => (p.gameId ?? "stealanegg") === "animeexpeditions"), [players]);
  ...
  } = useDashboardFilters(aePlayers, serverTime);

  // เรียงตาม AE เป็นหลัก: Level > Gold (ตั้งต้น ถ้า user ยังไม่แตะ sort)
  const sortedPlayers = useMemo(() => {
    if (sortField === "money" && sortDir === "desc") {
      return [...filteredPlayers].sort(
        (a, b) => (b.aeLevel ?? 0) - (a.aeLevel ?? 0) || (b.money ?? 0) - (a.money ?? 0)
      );
    }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#0b0d10] px-4 text-center">
        <div className="animate-enter w-full max-w-sm space-y-6 rounded-2xl border border-line bg-surface p-8 shadow-2xl">
          ...
          <div>
            <h2 className="text-xl font-black text-white">Anime Expeditions Dashboard</h2>
            <p className="mt-1.5 text-xs text-zinc-400">Sign in with Discord to access your expedition teams, unit vault, and currency stats.</p>
          </div>
          <button
            onClick={() => signIn("discord", { callbackUrl: "/animeexpeditions" })}
  ...
  return (
    <div className="min-h-screen bg-[#0b0d10] text-foreground flex flex-col animate-enter">
      <DashboardHeader
        ...
        gameIcon="fa-compass"
        gameLogo="/images/anime-expeditions/logo.png"
      ...
      <main className="mx-auto flex w-full max-w-[1760px] flex-1 flex-col px-4 py-6 sm:px-6 sm:py-8">
        {aePlayers.length === 0 ? (
          <div className="flex min-h-[40vh] flex-col items-center justify-center py-20 text-center">
            <img src="/images/anime-expeditions/logo.png" alt="Anime Expeditions" className="h-14 w-14 rounded-2xl border border-line object-cover" />
          ...
          <div className="space-y-6 pb-6">
            <div className="animate-enter" style={{ animationDelay: "0ms" }}>
              <AEStatsOverview players={aePlayers} />
            </div>
            <div className="animate-enter" style={{ animationDelay: "60ms" }}>
              <AEUnitVault players={aePlayers} onSelectGroup={setSelectedGroup} onSelectSkin={setSelectedSkin} />
            </div>
